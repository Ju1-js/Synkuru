...
async function getUserLists(token) {
  const query =
    "query($userId:Int){MediaListCollection(userId:$userId,type:ANIME,forceSingleCompletedList:true){lists{status,entries{media{id,format,title{userPreferred},coverImage{extraLarge},description,genres,averageScore,relations{edges{relationType,node{id}}}}}}}}";
...
async function getAnilistEntry(name, token) {
  ...
  const query =
    "query($search:String,$perPage:Int){Page(perPage:$perPage,page:1){media(type:ANIME,search:$search){id,format,title{userPreferred},coverImage{extraLarge},description,genres,averageScore}}}";
  const data = await getCachedResult(query, variables, () =>
    makeGraphQLRequest(query, variables, token)
  );
...
async function getAnilistEntryById(id, token) {
  ...
  const query =
    "query($mediaId:Int){MediaList(mediaId:$mediaId){progress status media{episodes format}}}";
  const data = await getCachedResult(query, variables, () =>
    makeGraphQLRequest(query, variables, token)
  );
...
async function getCatalog(catalogType, token) {
  ...
  if (variables.status) {
    query =
      "query($userId:Int,$sort:[MediaListSort]){MediaListCollection(userId:$userId,type:ANIME,sort:$sort,forceSingleCompletedList:true){lists{status,entries{media{id,format,title{userPreferred},coverImage{extraLarge},genres,averageScore}}}}}";
    ...
    query =
      "query($status_in:[MediaStatus],$id_in:[Int],$id_not_in:[Int],$sort:[MediaSort]){Page{media(type:ANIME,status_in:$status_in,id_in:$id_in,id_not_in:$id_not_in,format_not:MUSIC,sort:$sort){id,format,title{userPreferred},coverImage{extraLarge},genres,averageScore}}}";
    ...
    query =
      "query($sort:[MediaSort],$search:String,$genre:[String],$season:MediaSeason,$year:Int,$format_not:MediaFormat){Page{media(type:ANIME,sort:$sort,search:$search,genre_in:$genre,season:$season,seasonYear:$year,format_not:$format_not){id,format,title{userPreferred},coverImage{extraLarge},genres,averageScore}}}";
    const data = await getCachedResult(query, variables, () =>
      makeGraphQLRequest(query, variables, token)
    );
    const entries = data.data.Page.media;

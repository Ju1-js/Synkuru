...
function getReleaseInfo(media) {
  if (media.format === "MOVIE") {
    ...
    return `Coming ${media.startDate.year}`;
  } else if (media.status === "CANCELLED") {
    return `Cancelled (${media.startDate.year})`;
  } else if (media.status === "HIATUS") {
    return `On Hiatus (${media.startDate.year}${
...
function getReleasedInfo(media) {
  if (!media.startDate) return null;
  if (media.startDate.year) {
    let dateParts = [media.startDate.year];
    if (media.startDate.month) {
      dateParts.push(String(media.startDate.month).padStart(2, "0"));
      if (media.startDate.day) {
        dateParts.push(String(media.startDate.day).padStart(2, "0"));
      }
    }
  ...
  if (variables.status) {
    ...
    return Promise.all(
      entries.map(async (media) => ({
        ...
        released: getReleasedInfo(media),
